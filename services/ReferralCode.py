...
class ReferralCodeService:
    ...
    async def create_code(
        ...
        if check is not None:
            raise HTTPException(status_code=409, detail='You already have a referral code.')
        res = await self.referral_code_repository.create_code(session, referral_code, creator_id)
        return res
    ...
    async def delete_code(
        ...
        if code is None:
            raise HTTPException(status_code=409, detail="You don't have a referral code")
